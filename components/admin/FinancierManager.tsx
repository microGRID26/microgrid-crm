...
export function FinancierManager({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  ...
  const save = async () => {
    ...
    setSaving(false); setEditing(null); setToast('Financier saved'); setTimeout(() => setToast(''), 2500); load()
  }
  ...
  const createNew = async () => {
    ...
    setSaving(false); setShowNew(false); setDraft({}); setToast('Financier created'); setTimeout(() => setToast(''), 2500); load()
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-semibold text-white">Financier Manager</h2>
          <p className="text-xs text-gray-500 mt-0.5">{financiers.length} financier records</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-64"><SearchBar value={search} onChange={setSearch} placeholder="Search financiers..." /></div>
          <button onClick={() => { setShowNew(true); setDraft({}) }} className="px-3 py-1.5 text-xs bg-green-700 text-white rounded-md hover:bg-green-600">+ New Financier</button>
        </div>
      </div>
      <div className="flex-1 overflow-auto rounded-lg border border-gray-800">
        <table className="w-full text-xs">
          ...
          <tbody>
            ...
            {financiers.length === 0 && <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-600 text-sm">No financiers found</td></tr>}
          </tbody>
        </table>
      </div>
      {(editing || showNew) && (
