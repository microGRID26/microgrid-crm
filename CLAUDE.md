...
- `lib/api/commissions.ts` — `loadCommissionRates(orgId?)`, `addCommissionRate`, `updateCommissionRate`, `deleteCommissionRate`, `calculateCommission` (pure function — system watts, adder revenue, referral count, role key, rates array), `loadCommissionRecords(filters?)`, `createCommissionRecord`, `updateCommissionRecord`, `loadEarningsSummary(userId?, orgId?, dateRange?)`, `generateProjectCommissions(projectId, orgId?)` (reads project + adders, creates records for each applicable role). Constants: `COMMISSION_STATUSES`, `COMMISSION_STATUS_LABELS`, `COMMISSION_STATUS_BADGE`, `DEFAULT_ROLES`. Types re-exported from `types/database.ts`: `CommissionRate`, `CommissionRecord`, `CommissionStatus`, `CommissionRateType`. Also exports: `CommissionBreakdown`, `CommissionRecordFilters`, `EarningsSummary`.
...
- `components/Nav.tsx` — two-tier navigation bar. 8 primary links always visible (Command, Queue, NTP, Pipeline, Schedule, Funding, Analytics, Inventory) + "More" dropdown grouped into 5 sections: Operations (Service, Work Orders, Change Orders, Engineering, Audit), Finance (Commissions, Invoices), Supply Chain (Vendors), Tools (Atlas, Permits, Warranty, Fleet, Documents), Design (Redesign, Legacy). Audit Trail link in More dropdown for admins. Mobile drawer also uses section headers. Right-side slot for page controls.
